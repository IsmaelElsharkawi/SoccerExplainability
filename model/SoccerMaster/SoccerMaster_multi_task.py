...
from model.SoccerMaster.SoccerMaster import SoccerMaster
from model.SoccerMaster.SoccerMaster_caption_classification import build_caption_classification_head
...
class MultiTaskingModel(nn.Module):
    def __init__(self, config, logger=None):
        ...
        for head in all_heads:
            if head == "CaptionClassification":
                self.multi_task_head[head] = build_caption_classification_head(config)
            else:
                raise ValueError(f"Head {head} is not supported in this stripped-down copy. "
                                 f"Only 'CaptionClassification' is available.")
